...
Hull QuadCollisionShape::m_QuadHull = Hull();

QuadCollisionShape::QuadCollisionShape()
{
	m_QuadHalfDimensions = Vector3(0.5f, 0.5f, 0.5f);

	if (m_QuadHull.GetNumVertices() == 0)
	{
		ConstructCubeHull();
	}
}

QuadCollisionShape::QuadCollisionShape(const Vector3& halfdims)
{
	m_QuadHalfDimensions = halfdims/2;
...
Matrix3 QuadCollisionShape::BuildInverseInertia(float invMass) const
{
	Matrix3 inertia;

	Vector3 dimsSq = (m_QuadHalfDimensions + m_QuadHalfDimensions);
...
void QuadCollisionShape::GetCollisionAxes(const PhysicsObject* currentObject, std::vector<Vector3>* out_axes) const
{
	if (out_axes)
	{
...
void QuadCollisionShape::GetEdges(const PhysicsObject* currentObject, std::vector<CollisionEdge>* out_edges) const
{
	if (out_edges)
	{
		Matrix4 transform = currentObject->GetWorldSpaceTransform() * Matrix4::Scale(Vector3(m_QuadHalfDimensions));
		for (unsigned int i = 0; i < m_QuadHull.GetNumEdges(); ++i)
		{
			const HullEdge& edge = m_QuadHull.GetEdge(i);
			Vector3 A = transform * m_QuadHull.GetVertex(edge.vStart).pos;
			Vector3 B = transform * m_QuadHull.GetVertex(edge.vEnd).pos;
...
void QuadCollisionShape::GetMinMaxVertexOnAxis(
	...
	Vector3* out_max) const
{
	...
	Matrix4 wsTransform = currentObject->GetWorldSpaceTransform() * Matrix4::Scale(m_QuadHalfDimensions);
	...
	int vMin, vMax;
	m_QuadHull.GetMinMaxVerticesInAxis(local_axis, &vMin, &vMax);

	// Return closest and furthest vertices in world-space
	if (out_min) *out_min = wsTransform * m_QuadHull.GetVertex(vMin).pos;
	if (out_max) *out_max = wsTransform * m_QuadHull.GetVertex(vMax).pos;
}

void QuadCollisionShape::GetIncidentReferencePolygon(
	const PhysicsObject* currentObject,
	const Vector3& axis,
	...
	std::vector<Plane>* out_adjacent_planes) const
{
	...
	m_QuadHull.GetMinMaxVerticesInAxis(local_axis, &undefined, &maxVertex);
	const HullVertex& vert = m_QuadHull.GetVertex(maxVertex);
	...
	{
		const HullFace* face = &m_QuadHull.GetFace(faceIdx);
	...
	if (out_face)
	{
		...
		{
			const HullVertex& vert = m_QuadHull.GetVertex(vertIdx);
	...
	{
		Vector3 wsPointOnPlane = wsTransform * m_QuadHull.GetVertex(m_QuadHull.GetEdge(best_face->edge_ids[0]).vStart).pos;
		...
		{
			const HullEdge& edge = m_QuadHull.GetEdge(edgeIdx);

			wsPointOnPlane = wsTransform * m_QuadHull.GetVertex(edge.vStart).pos;

			for (int adjFaceIdx : edge.enclosing_faces)
			{
				if (adjFaceIdx != best_face->idx)
				{
					const HullFace& adjFace = m_QuadHull.GetFace(adjFaceIdx);
...
void QuadCollisionShape::DebugDraw(const PhysicsObject* currentObject) const
{
	// Just draw the cuboid hull-mesh at the position of our PhysicsObject
	Matrix4 transform = currentObject->GetWorldSpaceTransform() * Matrix4::Scale(m_QuadHalfDimensions);
	m_QuadHull.DebugDraw(transform);
}

void QuadCollisionShape::ConstructCubeHull()
{
	//Vertices
	m_QuadHull.AddVertex(Vector3(-1.0f, -1.0f, -1.0f));		// 0
	m_QuadHull.AddVertex(Vector3(-1.0f, 1.0f, -1.0f));		// 1
	m_QuadHull.AddVertex(Vector3(1.0f, 1.0f, -1.0f));		// 2
	m_QuadHull.AddVertex(Vector3(1.0f, -1.0f, -1.0f));		// 3

	m_QuadHull.AddVertex(Vector3(-1.0f, -1.0f, 1.0f));		// 4
	m_QuadHull.AddVertex(Vector3(-1.0f, 1.0f, 1.0f));		// 5
	m_QuadHull.AddVertex(Vector3(1.0f, 1.0f, 1.0f));		// 6
	m_QuadHull.AddVertex(Vector3(1.0f, -1.0f, 1.0f));		// 7
	...
	//Faces
	m_QuadHull.AddFace(Vector3( 0.0f,  0.0f, -1.0f), 4, face1);
	m_QuadHull.AddFace(Vector3( 0.0f,  0.0f,  1.0f), 4, face2);
	m_QuadHull.AddFace(Vector3( 0.0f,  1.0f,  0.0f), 4, face3);
	m_QuadHull.AddFace(Vector3( 0.0f, -1.0f,  0.0f), 4, face4);
	m_QuadHull.AddFace(Vector3( 1.0f,  0.0f,  0.0f), 4, face5);
	m_QuadHull.AddFace(Vector3(-1.0f,  0.0f,  0.0f), 4, face6);
}
